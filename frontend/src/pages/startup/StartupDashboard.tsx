...
export const StartupDashboard: React.FC = () => {
  ...
  const startupAppForTab = myApplications.find((app) => {
    ...
    if (activeTab === "selected") {
      ...
      return app.status === "Approved" || !!hasSelectedPref;
    }
    if (activeTab === "rejected") {
      const hasRejectedPref = app.incubatorPreferences?.some((p) => p.status === "Rejected");
    ...
    <div className="space-y-8" id="startup-dashboard-container">
      ...
      <section className="relative left-1/2 -mt-8 w-screen -translate-x-1/2 bg-slate-50 pt-8">
        <div className="mx-auto max-w-[1600px] px-4 py-8 sm:px-6 lg:px-8">
          {latestApplication && latestApplication.programId === "startup-program" ? (
            <div className="space-y-6" id="startup-program-tracking-view">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-200 pb-2 mb-6 gap-4">
                ...
                <Link
                  to="/programs"
                  className="bg-[#F9B233] hover:bg-[#e09e2b] text-slate-950 font-bold px-6 py-2 rounded-full text-xs uppercase tracking-wider transition-colors shadow-sm"
                >
                  Apply Now
                </Link>
              </div>
              ...
              {startupAppForTab ? (
                          ...
                          <div key={pref.preferenceOrder} className="p-6 space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-xl text-xs pt-1">
                              <div className="space-y-1">
                                ...
                                <p className="font-bold text-slate-800">{pref.submittedDate || startupAppForTab.submittedDate}</p>
                              </div>
                              <div className="space-y-1">
                                <span className="text-slate-400 font-medium">Application Completeness Checked On</span>
                                <p className="font-bold text-slate-750">{pref.completenessStatus || "In Progress"}</p>
                              </div>
                            </div>
                                  ...
                                  <table className="w-full text-[11px] text-left border-t border-slate-100">
                                    ...
                                    <tbody>
                                      <tr className="text-slate-700 font-medium">
                                        <td className="py-2.5 pr-4 font-bold text-slate-800">{pref.status}</td>
                                        <td className="py-2.5 px-4 font-mono text-[10px] text-slate-500">
                                          {pref.commentsDate || pref.submittedDate || startupAppForTab.submittedDate}
                                        </td>
                ...
                <div className="bg-white border border-slate-200 rounded-lg p-10 text-center shadow-xs">
                  <p className="text-sm font-semibold text-slate-500">
                    No applications found in the "{activeTab === "in_progress" ? "In Progress" : activeTab === "selected" ? "Selected" : activeTab === "closed" ? "Closed / Cancelled" : "Rejected"}" tab.
                  </p>
                </div>
              )}
            </div>
          ) : (
            // ── Original layout kept as-is for the other 4 programs ──
            <>
