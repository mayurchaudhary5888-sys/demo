...
const initialFields = (
  ...
  authorFirstName: user?.name?.split(" ")?.[0] || "",
  authorLastName: user?.name?.split(" ")?.slice(1).join(" ") || "",
  designation: "",
  mobile: "",
  email: user?.email || "",
  dpiitNumber: user?.startupId || "",
  ...
  websiteUrl: "",
  linkedInUrl: "",
  teamMembers: [{ name: "", role: "", email: "", mobile: "" }],
  fullTimeEmployees: 1,
  raisedFunding: "No",
  fundingAmount: "",
  fundingInstrument: "",
...
export const StartupProgramApplication: React.FC<StartupProgramApplicationProps> = ({ program, onCancel }) => {
  ...
  const [fields, setFields] = useState<StartupWizardFields>(() => initialFields(user || undefined, userStartup?.startupName || userStartup?.name || ""));
            ...
            <div className="space-y-6">
              {step === 0 && (
                <StepSection title="Authorized Representative" subtitle="Who is filing this application?">
                  <div className="grid gap-5 md:grid-cols-2">
                    <Input label="First Name *" value={fields.authorFirstName} onChange={(value) => updateField("authorFirstName", value)} error={errors.authorFirstName} />
                    <Input label="Last Name *" value={fields.authorLastName} onChange={(value) => updateField("authorLastName", value)} error={errors.authorLastName} />
                    <Input label="Designation *" value={fields.designation} onChange={(value) => updateField("designation", value)} error={errors.designation} />
                    <Input label="Mobile No. *" value={fields.mobile} onChange={(value) => updateField("mobile", value)} error={errors.mobile} />
                    <Input label="Email ID *" value={fields.email} onChange={(value) => updateField("email", value)} error={errors.email} className="md:col-span-2" />
                  </div>
                </StepSection>
              )}
              ...
              {step === 1 && (
                <StepSection title="Entity Details" subtitle="Capture your company identity and registration details.">
                  <div className="grid gap-5 md:grid-cols-2">
                    <Input label="DPIIT Recognition Number" value={fields.dpiitNumber} onChange={(value) => updateField("dpiitNumber", value)} />
                    <Input label="Name of the Entity *" value={fields.entityName} onChange={(value) => updateField("entityName", value)} error={errors.entityName} />
                    <Select label="Nature of Entity *" value={fields.natureOfEntity} onChange={(value) => updateField("natureOfEntity", value)} error={errors.natureOfEntity} options={entityOptions} />
                    <Input label="Incorporation / Registration Date *" type="date" value={fields.incorporationDate} onChange={(value) => updateField("incorporationDate", value)} error={errors.incorporationDate} />
                    <Input label="PAN Number *" value={fields.panNumber} onChange={(value) => updateField("panNumber", value)} error={errors.panNumber} />
                    <Input label="State *" value={fields.state} onChange={(value) => updateField("state", value)} error={errors.state} />
                    <Input label="City *" value={fields.city} onChange={(value) => updateField("city", value)} error={errors.city} />
                    <Input label="Startup Address *" value={fields.address} onChange={(value) => updateField("address", value)} error={errors.address} />
                  </div>
                </StepSection>
              )}
              ...
              {step === 2 && (
                <StepSection title="Startup Details" subtitle="Summarize what you build, for whom, and why it matters.">
                  <div className="grid gap-5">
                    <Toggle label="Is it a Technology Startup? *" value={fields.technologyStartup} onChange={(value) => updateField("technologyStartup", value)} />
                    <TextArea label="What is the problem you are solving? *" value={fields.problemStatement} onChange={(value) => updateField("problemStatement", value)} error={errors.problemStatement} />
                    <TextArea label="What is your value proposition? *" value={fields.valueProposition} onChange={(value) => updateField("valueProposition", value)} error={errors.valueProposition} />
                    <TextArea label="What is your unique selling point? *" value={fields.uniqueSellingPoint} onChange={(value) => updateField("uniqueSellingPoint", value)} error={errors.uniqueSellingPoint} />
                    <TextArea label="What is your target customer segment? *" value={fields.targetCustomer} onChange={(value) => updateField("targetCustomer", value)} error={errors.targetCustomer} />
                    <Input label="What is the market size of the opportunity? *" value={fields.marketSize} onChange={(value) => updateField("marketSize", value)} error={errors.marketSize} />
                    <TextArea label="How do you aim to scale-up? *" value={fields.scalePlan} onChange={(value) => updateField("scalePlan", value)} error={errors.scalePlan} />
                    <TextArea label="What will be the revenue model? *" value={fields.revenueModel} onChange={(value) => updateField("revenueModel", value)} error={errors.revenueModel} />
                    <Input label="Who are your key competitors?" value={fields.competitors} onChange={(value) => updateField("competitors", value)} />
                    <Input label="Website URL (Optional)" value={fields.websiteUrl} onChange={(value) => updateField("websiteUrl", value)} />
                  </div>
                </StepSection>
              )}
                ...
                <StepSection title="Startup Team" subtitle="List the core people working on the startup.">
                  <div className="space-y-4">
                    <div className="grid gap-5 md:grid-cols-2">
                      ...
                      <Input label="LinkedIn Profile (Optional)" value={fields.linkedInUrl} onChange={(value) => updateField("linkedInUrl", value)} />
                    </div>
                      ...
                      <div className="mt-4 space-y-3">
                        {fields.teamMembers.map((member, index) => (
                          <div key={index} className="grid gap-3 md:grid-cols-[1.2fr_1fr_1.1fr_0.9fr_auto]">
                            <Input label="Name of Promoter" value={member.name} onChange={(value) => updateTeamMember(index, "name", value)} />
                            <Input label="Role" value={member.role} onChange={(value) => updateTeamMember(index, "role", value)} />
                            <Input label="Email" value={member.email} onChange={(value) => updateTeamMember(index, "email", value)} />
                            <Input label="Mobile" value={member.mobile} onChange={(value) => updateTeamMember(index, "mobile", value)} />
                    ...
                    <div className="grid gap-5 md:grid-cols-2">
                      <div className="rounded-2xl border border-slate-200 bg-white p-4">
                        ...
                        <div className="mt-3 flex items-center gap-3">
                          <button type="button" onClick={() => updateField("fullTimeEmployees", Math.max(1, fields.fullTimeEmployees - 1))} className="h-8 w-8 rounded-full border border-[#F05A28] text-[#F05A28]">-</button>
                          <span className="min-w-12 rounded-lg bg-slate-100 px-4 py-2 text-center font-bold">{fields.fullTimeEmployees}</span>
                          <button type="button" onClick={() => updateField("fullTimeEmployees", fields.fullTimeEmployees + 1)} className="h-8 w-8 rounded-full border border-[#F05A28] text-[#F05A28]">+</button>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              {step === 4 && (
                <StepSection title="Funding Details" subtitle="Share the current capital and funding profile.">
                  <div className="grid gap-5 md:grid-cols-2">
                    <Toggle label="Have you cumulatively received more than ₹10 lakhs? *" value={fields.raisedFunding} onChange={(value) => updateField("raisedFunding", value)} />
                    <Input label="Current Funding Requirement" value={fields.fundingAmount} onChange={(value) => updateField("fundingAmount", value)} error={errors.fundingAmount} />
                    <Select label="Funding Instrument" value={fields.fundingInstrument} onChange={(value) => updateField("fundingInstrument", value)} error={errors.fundingInstrument} options={fundingInstrumentOptions} />
                    <Input label="Your current revenue / traction note" value={fields.revenueModel} onChange={(value) => updateField("revenueModel", value)} />
                  </div>
                </StepSection>
              )}
              ...
              {step === 6 && (
                <StepSection title="Upload Documents" subtitle="Final attachments before submit.">
                  <div className="grid gap-6">
                    ...
                    />
                    <Input label="Video URL showcasing your product and/or business model (Optional)" value={fields.videoUrl} onChange={(value) => updateField("videoUrl", value)} />
                    <FileUpload
...
}> = ({ label, helper, sampleText, file, kind, onFile }) => (
  <div className="space-y-2">
    ...
    <div className="rounded-2xl border-2 border-dashed border-[#FDBA74] bg-[#FFF8F0] p-5 text-center">
      <input
        ...
        accept=".pdf,.ppt,.pptx"
      />
      <label htmlFor={`${kind}-file-input`} className="inline-flex cursor-pointer items-center gap-2 rounded-full border border-[#F05A28] px-5 py-3 text-xs font-black uppercase tracking-wider text-[#F05A28]">
